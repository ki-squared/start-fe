...
var lastPage = Math.ceil(todayPhoto.length/carouselLength);

show();

function show() {
    var imgStr = '';
    var firstInPage = (pageNumber-1)*3;

    for(var i=firstInPage; i<firstInPage+carouselLength; i++) {
        if(i===todayPhoto.length) {
            break;
        }
        imgStr += `<img src = "${todayPhoto[i].img}">`;
    }

    $wrap.innerHTML = imgStr;
    $page.innerHTML = `${pageNumber}/3`;
}
